/**
 * Form rules: conditional visibility, whole-form validation, and the
 * publish-time configuration lint.
 *
 ...
 *
 * This module is deliberately PURE: no D1, no Env, no error logging, no
 * Cloudflare types. The browser imports it directly so the applicant form
 * hides, reveals and validates fields using exactly the code the Worker will
 * use on submit -- a form the browser accepts and the server rejects is a bug
 * report from an applicant at 11pm on deadline day. Anything that needs the
 * database lives in loadForm.ts.
 */
